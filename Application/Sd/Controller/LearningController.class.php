...
class LearningController extends CommonController {
	...
	public function learningPreschoolAct()
	{
		...
		if(!empty($course[rows]))
		{
			...
			//统计进度
			$progressPreschool = D('Learning', 'Logic')->statisticsDataPreschool($courseIds,$this->role['id'],120);
	...
	*/
	public function detailAct()
	{
		...
		$pageHtml = get_page_html($page, $pageCount);
		//$page = get_pageHtml2($total,12,array(),'/static/v1/Sd/images/common/page');
...
//		dump($this->progressPreschool);exit;
		...
		{
			$data[$key]['name'] = $value['name'];
			$data[$key]['length'] = empty($progressPreschool[$value['courseId']][$value['id']]) ? 0 : $progressPreschool[$value['courseId']][$value['id']]*158;
		}
		$score = Session('score');
	...
	private function getProgressOfEarly($total, $finish)
	{
		...
		foreach ($total as $key => $value)
		{
			...
			{
				$length[$key] = ($total[$key]-$finish[$key])*234/$total[$key];
			}
		}
		return $length;
	}
	...
	private function initUserCenterChannel(){
		...
		foreach($channel as $key => $value)
		{
			if($value['chKey'] == 'learning')
			{
				...
				{
					$channel[$key]['linkUrl'] = 'Learning/learningEarly?arrange=month';
				}
				else
				{
					$channel[$key]['linkUrl'] = 'Learning/learningPreschool';
				}
			}
		}
		//把栏目的图片数组拆开
