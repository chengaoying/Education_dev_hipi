...
<script type="text/javascript">
	...
	function initbuttons()
	{
		for(var i=1; i<=buttons.length-4; i++)
		{
			...
			obj.resize = -1;
			obj.right = (i<buttons.length-2) ? 'option_'+(i+1) : '' ; 
			obj.left = (i>1) ? 'option_'+(i-1) : '' ; 
			obj.down = (i+colnum<=buttons.length-2) ? 'option_'+(i+colnum) : '';
		...
		buttons[buttons.length-4-1]['down'] = 'page_prev';
		buttons[buttons.length-4-2]['down'] = 'page_prev';
