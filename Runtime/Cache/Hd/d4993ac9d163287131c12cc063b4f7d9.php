...
var buttons=
	[
        ...
        {id:'stage_3',name:'',action:'',linkImage:'',focusImage:'',selectBox:'',left:'stage_2',right:'',up:'',down:'course_2'},
        
        /* 课程列表 */
        {id:'course_1',name:'',action:'',linkImage:'',focusImage:'',selectBox:'/static/v1/hd/images/common/selectBox/select_box_210x210.gif',left:'stage_1',right:'course_2',up:'stage_1',down:'course_6'},
...
<!-- 早教龄段列表 -->
<?php if(is_array($stageList)): $i = 0; $__LIST__ = $stageList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$stage): $mod = ($i % 2 );++$i; $left = 270+($i-1)*90; ?>
    <div id="div_stage_<?php echo ($i); ?>" title="<?php echo U('CourseList/early?stageId='.$i);?>" style="position:absolute;width:80px;height:38px;left:<?php echo ($left); ?>px;top:80px;text-align:center;">
        <img id="stage_<?php echo ($i); ?>" src="<?php echo ($stage['linkImage']); ?>" width="61" height="26">
...
<?php if(is_array($courseList)): $i = 0; $__LIST__ = $courseList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$course): $mod = ($i % 2 );++$i; if($i > 5){ $top = 415; $left = 80 + ($i-6)*225; }else{ $top = 182; $left = 80 + ($i-1)*225; } ?>
    <div id="div_course_<?php echo ($i); ?>" title="<?php echo U('SectionList/early');?>" style="position:absolute;width:220px;height:220px;left:<?php echo ($left); ?>px;top:<?php echo ($top); ?>px;text-align:center;">
