...
function initPlayer(playUrl, width, height, left, top) {
    ...
    var muteFlag = 1; 				//0: 设置为有声 (默认值) 1: 设置为静音
    var subtitleFlag = 0; 			//字幕显示
    var videoAlpha = 0;				//视频的透明度
    var cycleFlag = 0;				//设置是否循环播放节目。0: 设置为循环播放（默认值）1：设置为单次播放
    ...
    mp.setNativeUIFlag(1); 								//设置播放器本地UI显示功能 0:允许 1：不允许
